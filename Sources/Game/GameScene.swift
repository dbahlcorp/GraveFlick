...
final class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    /// Zombie grab/drag/flick input — touch tracking, dual-touch pinch/rotate, release velocity.
    private let interactionController = InteractionController()
    ...
    /// Gore/hit-flash/debris VFX, pavement cracks, and diner siding damage.
    let combatVFXSystem = CombatVFXSystem()
    ...
    var flickMultiplier: CGFloat { (1 + CGFloat(progress.upgradeLevel(.flickTraining)) * 0.10) * perkSystem.flickVelocityBonus }
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if wave != lastObservedWaveForFeats {
            if lastObservedWaveForFeats > 0, interactionController.playerThrewThisWave, !interactionController.playerThrownZombieGroundedThisWave {
                recordFeat("graceful_wave")
            }
            lastObservedWaveForFeats = wave
            interactionController.playerThrewThisWave = false
            interactionController.playerThrownZombieGroundedThisWave = false
        }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        interactionController.touchesBegan(touches, in: self)
    }

    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        interactionController.touchesMoved(touches, in: self)
    }

    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        interactionController.touchesEnded(touches, in: self)
    }

    override func touchesCancelled(_ touches: Set<UITouch>, with event: UIEvent?) {
        interactionController.touchesCancelled(touches, in: self)
    }
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if let zombie = zombieBody(in: bodies), bodies.contains(where: { $0.categoryBitMask == PhysicsCategory.ground }), zombie.isThrown, !zombie.hasResolvedImpact {
            ...
            if zombie.wasPlayerThrown { interactionController.playerThrownZombieGroundedThisWave = true }
    ...
    private func resolveImpact(on zombie: ZombieNode, damage rawDamage: CGFloat) {
        ...
        combatVFXSystem.spawnPavementCracks(at: impactPoint, power: power)
        if power > 0.3 {
    ...
    private func resolveDinerAttack(from zombie: ZombieNode) {
        ...
        combatVFXSystem.damageScenery(near: zombie.position)
    ...
    /// Forwards to `combatVFXSystem` — kept as a same-named GameScene member because
    /// ComboSystemHost (which predates CombatVFXSystem's extraction) still requires it.
    func spawnPhysicsDebris(from zombie: ZombieNode, reason: DefeatReason) {
        combatVFXSystem.spawnPhysicsDebris(from: zombie, reason: reason)
    }
    ...
    func finish(won: Bool) {
        ...
        interactionController.releaseAll()
    ...
    /// Forwards to `combatVFXSystem` — kept as same-named GameScene members because
    /// ComboSystemHost/WeaponSystemHost/BossDirectorHost (which predate CombatVFXSystem's
    /// extraction) still require them.
    func burst(at point: CGPoint, color: SKColor) { combatVFXSystem.burst(at: point, color: color) }

    func playCombatVFX(_ effect: CombatVFX, at point: CGPoint, size: CGFloat, direction: CGFloat, tint: SKColor? = nil) {
        combatVFXSystem.playCombatVFX(effect, at: point, size: size, direction: direction, tint: tint)
    }

    func playDirectionalDebris(at point: CGPoint, color: SKColor, direction: CGFloat, count: Int) {
        combatVFXSystem.playDirectionalDebris(at: point, color: color, direction: direction, count: count)
    }
...
extension GameScene: InteractionControllerHost {}
extension GameScene: CombatVFXSystemHost {}
